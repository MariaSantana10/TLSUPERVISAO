...
metadados = {
    "id_sensor": int(os.getenv('SENSOR_ID', 3)),
    "localizacao": os.getenv('LOCALIZACAO', 'Sala'),
    'tipo': 'simulado',
    'unidade': 'True or False',
    'natureza_sensor': 'eletromagnética',
    'variavel': 'movimento',
    'versao_servidor': '0.1',
...
MQTT_TOPIC = os.getenv(
    'MQTT_TOPIC', 
    f"sensor/lampada/sala{metadados['id_sensor']}"
)
PUBLISH_INTERVAL_SECONDS = int(os.getenv('PUBLISH_INTERVAL_SECONDS', 20))

def on_connect(client, userdata, flags, rc):
    if rc == 0:
        print(f'Conectado ao broker em {MQTT_BROKER_HOST}:{MQTT_BROKER_PORT}')
    else:
        print(f'Falha na conexão como broker, código de retorno: {rc}')
...
@app.route('/')
def home():
    return "API de dados do sensor de umidade"
